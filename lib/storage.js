...
function hasLocal(moduleItem, name) {
  ...
  if (moduleLocal) {
    moduleItem = Object.assign(moduleItem, JSON.parse(Decrypt(moduleLocal)) // JSON.parse(moduleLocal)
    );
  } else {
...
export function setLocal(name, module, stringify) {
  ...
  if (list) {
    ...
    list.forEach(function (key) {
      Reflect.set(obj, key, Reflect.get(stringify ? JSON.parse(module) : module, key));
    }); // 将他们存放到Localstorage中

    Local.setItem(name, Encrypt(JSON.stringify(obj))); // Local.setItem(name, JSON.stringify(obj));
  }
}
export function setSession(name, module, stringify) {
